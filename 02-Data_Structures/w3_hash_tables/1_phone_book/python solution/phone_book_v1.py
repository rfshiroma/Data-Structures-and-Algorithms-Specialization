...
class PhoneBook:
    ...
    def __init__(self):
        self.book = [None] * 10000000           # optimal memory usage

    def add(self, number, name):
        '''Adds a person with name and phone number to the phone book.

        If there exists a user with such number already, then manager overwrites the corresponding name.
        '''
    ...
    def find(self, number):
        ...
        Replies with the person's name, or with string "not found" (without quotes) if there is no such person in the book.
        '''
...
def process_queries(queries):
    '''Helper function which reads queries from standard input, runs phonebook manager and sends the results to standard output.
    '''
